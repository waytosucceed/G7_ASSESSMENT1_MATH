...
function displayResults() {
  ...
  for (var page = 0; page < numberOfPages; page++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      var ques = quesgroup.question;
      var ans = quesgroup.options[quesgroup.answer];
      var ansContent = ans.image ? "<img src='" + ans.image + "' alt='Answer Image' style='width: 250px; height: 120px;'>" : getOptionLabel(ans);
      var givenAnswer = Ansgiven[j] !== undefined ? quesgroup.options[Ansgiven[j]] : null;
      var givenContent = "";

      // Style answers based on correctness and if answered
      if (givenAnswer) {
        if (Ansgiven[j] === null) {
          givenContent = "<span style='color: red;'>Not Answered</span>";
        } else if (Ansgiven[j] !== quesgroup.answer) {
          givenContent = "<span style='color: red;'>" + (givenAnswer.image ? "<img src='" + givenAnswer.image + "' alt='Given Answer Image' style='width: 250px; height: 120px;'>" : getOptionLabel(givenAnswer)) + "</span>";
        } else {
          givenContent = givenAnswer.image ? "<img src='" + givenAnswer.image + "' alt='Given Answer Image' style='width: 250px; height: 120px;'>" : getOptionLabel(givenAnswer);
        }
      } else {
        givenContent = "<span style='color: red;'>Not Answered</span>";
      }
